...
export default function byFriend() {
    const params = useParams()
    const friend = decodeURIComponent(params.friend)

    const [musicList, setMusicList] = useState([]);
    const getMusicList = async () => {
        const response = await fetch(`https://kubook-exp.shop/music/by-friend/${friend}`);
    ...
    return (
        <div className="min-h-screen bg-background bg-[url('/media/time_background.png')] bg-cover bg-center bg-no-repeat bg-fixed">
            ...
            <div className="grid grid-cols-1 gap-8 mt-8 pr-7 pl-14">
                <h1 className="text-2xl font-bold pl-12">친구: {friend}</h1>
